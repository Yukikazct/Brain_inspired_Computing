"""
STDP核心模型 — Diehl & Cook (2015) 无监督 STDP 学习

LIF神经元 + 三重trace STDP + E↔I侧向抑制回路

用法:
  train_stdp.py 设置超参数后依次调用 train() → observe() → test()
"""
from brian2 import *
...
def stats(net):
    """收集网络运行状态: 仿真时刻、总发放数、权重/theta的均值和标准差"""
...
# ---- 三阶段 ----
def train():
    """STDP 无监督训练阶段: 循环展示训练样本, 通过 STDP 更新输入→兴奋突触权重"""
    X, Y = read_mnist(True)
    n_samples = X.shape[0]
...
def observe():
    """观察/标注阶段: 冻结权重, 统计每个数字类对应的平均发放模式, 生成神经元→类别的分配表"""
    X, Y = read_mnist(True)
    n_samples = X.shape[0]
