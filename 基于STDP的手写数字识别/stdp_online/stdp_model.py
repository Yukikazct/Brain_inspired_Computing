...
def read_mnist(training):
    ...
    images.read(4)                               # 跳过magic number
    n_images = unpack('>I', images.read(4))[0]    # 图像数量
    n_rows = unpack(">I", images.read(4))[0]      # 行数(=28)
    n_cols = unpack(">I", images.read(4))[0]      # 列数(=28)

    # 读取标签文件
    labels = open(MNIST_PATH / ('%s-labels-idx1-ubyte' % tag), 'rb')
    labels.read(8)                               # 跳过magic number + count
