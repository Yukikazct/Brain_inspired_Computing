...
加载预训练的STDP模型，执行三部分操作:
  ...
  2. 完整准确率 — 从预存的confusion.npy直接读取10K测试集结果（秒出）
  3. 可视化 — 生成感受野图 + 神经元分配统计图

为什么分成实时推理和读取混淆矩阵两部分？
  - Brian2的仿真速度较慢，逐张推理10K样本需要数分钟
  - confusion.npy是train_stdp.py在训练后一次性跑完的完整结果
  - 日常使用只需读confusion.npy（秒出准确率） + 少量实时推理演示

用法:
  python test_brian.py    # 需要已有的预训练权重和confusion.npy
"""
...
# ═══════════════════════════════════════════════════════════════
# 2. 完整准确率 — 从预存的混淆矩阵直接读取（秒级）
# ═══════════════════════════════════════════════════════════════
# confusion.npy: 10×10归一化混淆矩阵，由train_stdp.py测试阶段生成
# 每行和为1，对角元素 = 该类别的识别准确率
