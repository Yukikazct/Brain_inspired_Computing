 ...
 *  Masquelier T, Guyonneau R, Thorpe SJ (2009). Competitive STDP-based Spike Pattern Learning. Neural Computation: in press.
...
const int nNeuronField = 13; // the number of fields in neuronField 
const	char *neuronField[nNeuronField] = {
		"weight",
		"epspAmplitude",
		"epspTime",
		"epspAfferent",
...
/*  Not a good idea. Tim 12/2007. (*neuron).weight[afferent] += (*neuron).weight[afferent] * (1-(*neuron).weight[afferent]) * (*param).stdp_a_neg * exp(-(spikeTime-lastFiringTime) / (*param).stdp_t_neg); */
	...
	for(e=(int)*(*neuron).nEpsp;e>=1;e--) {
		...
		if(!alreadyPotentiated[epspAfferent]) {/* consider only pairs of spikes*/
			(*neuron).weight[epspAfferent] += (*param).stdp_a_pos * exp(-(firingTime-epspTime)/(*param).stdp_t_pos);
 			/*Not a good idea. Tim 12/2007. (*neuron).weight[epspAfferent] += (*neuron).weight[epspAfferent] * (1-(*neuron).weight[epspAfferent]) * (*param).stdp_a_pos * exp(-(firingTime-epspTime)/(*param).stdp_t_pos); */
        
			(*neuron).weight[epspAfferent] = local_min(1-(*param).minWeight,(*neuron).weight[epspAfferent]);
...
/* 	for(e=*(*neuron).nEpsp-1; e>*(*neuron).nInefficient-1; e--) { */
	...
	if( *(*neuron).nEpsp-1 - e >= .9*(*neuron).NEPSP) {
        mexPrintf("Current EPSP array size = %d",(*neuron).NEPSP);
