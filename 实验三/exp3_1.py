...
def alpha_n(V):
    x = V + 55.0
    # 当分母接近 0 时，直接使用极限值，避免数值不稳定
    # return 0.01 * x / (1 - np.exp(-x / 10))
    if abs(x) < 1e-6:
        return 0.1
    return 0.01 * x / (1 - np.exp(-x / 10))
...
# 本实验要求修改的参数
stim_amplitude = 10.0    # 外加电流幅值，决定刺激强弱
stim_start = 15.0    # 外加电流开始时刻（ms）
...
axes[1].axhline(0.0, color="gray", linestyle="--", alpha=0.7, label="0mV reference")
